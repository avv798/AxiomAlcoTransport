...
{
    public partial class WaybillRepealSingleForm : XtraForm
    {
        ...
        /// Только чтение.
        /// </summary>
        private readonly WaybillRepeal waybillRepeal;
        /// <summary>
        ...
        /// </summary>
        public WaybillRepealSingleForm()
        {
            try
            {
        ...
        public WaybillRepealSingleForm(Documents documents, WaybillRepeal waybillRepeal, bool isAccepted) : this()
        {
            try
            {
                ...
                this.waybillRepeal = waybillRepeal;
        ...
        private void WaybillRepealSingleForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            Program.Logger.Info(this, "Форма закрывается.");
        }
        private void WaybillRepealSingleForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Caption_labelControl.Text = string.Format("Создать и отправить документ {0} по запросу на распроведение накладной " +
                                                          "за номером '{1}' " +
                                                          "дата составления - '{2}', " +
                                                          "полученного '{3}' " +
                                                          "(номер накладной по документу регистрации движения - '{4}').",
                                                           isAccepted ? "подтверждения" : "отказа",
                                                           waybillRepeal.RequestNumber,
                                                           waybillRepeal.RequestDate,
                                                           waybillRepeal.CreateDateTime,
                                                           waybillRepeal.WBRegId);

                Number_textEdit.Text = string.Format("{0}-{1}-{2}", waybillRepeal.WBRegId,
                                                                    (isAccepted) ? "accepted" : "rejected",
                                                                    DateTime.Now.ToString("yyMMddHHmmss"));
                Comment_textEdit.Text = isAccepted ? "Подтверждение запроса." : string.Empty;
        ...
        private void Send_simpleButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                documents.SendConfirmWaybillRepeal(waybillRepeal, isAccepted, Number_textEdit.Text, Comment_textEdit.Text);
