...
{
    public partial class ActChargeOnEditorForm : XtraForm
    {
        ...
        /// Только чтение.
        /// </summary>
        private readonly ActChargeOn act;
        /// <summary>
        ...
        public ActChargeOnEditorForm(Documents documents, ActChargeOn act) : this()
        {
            try
            {
        ...
        private void ActChargeOnEditorForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                SourcePosition_bindingSource.DataSource = documents.Production;
        ...
        private void ActChargeOnEditorForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (dataChanged)
        ...
        private void ActChargeOnEditorForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
        ...
        private void fillFormByActInfo()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(act.Reason)) Base_comboBoxEdit.Text = act.Reason;
        ...
        private void fillActByFormInfo()
        {
            ...
            act.Reason = Base_comboBoxEdit.Text;
        ...
        private void setControlsEnable()
        {
            ...
            else
            {
                ...
                Help_simpleButton.Enabled = true;
                ...
                Text = "Просмотр акта постановки товара на баланс на склад организации";
            }
        }
        /// <summary>
        /// Добавить позицию.
        ...
        private void addPosition()
        {
            try
            {
                ...
                Production selected = (Production)SourcePosition_bindingSource.Current;

                if (selected == null) throw new Exception("Ошибка получения текущего документа.");

                ActChargeOnDetailForm detailForm = new ActChargeOnDetailForm(selected);

                if (detailForm.ShowDialog(this) == DialogResult.OK)
                {
                    #region Действия, если такая продукция уже присутствует в документе.

                    if (positions.Any(MovePosition => ((MovePosition.AlcoCode == selected.AlcCode) && (MovePosition.TTNNumber == detailForm.TtnNumber))))
                    {
                    ...
                    MovePosition movePosition = new MovePosition(MovementType.ActChargeOn, selected)
                                                    {
                                                        UseScan = detailForm.UseScan,
                                                        BottlingDate = detailForm.BottlingDate,
                                                        TTNNumber = detailForm.TtnNumber,
                                                        TTNDate = detailForm.TtnDate,
                                                        EGAISNumber = detailForm.EgaisNumber,
                                                        EGAISDate = detailForm.EgaisDate,
                                                        Quantity = detailForm.Quantity
                                                    };

                    if (detailForm.UseScan)
                    {
                        for(int i = 0; i < detailForm.Quantity; ++i)
                        {
                            PDF417RequestForm pdf417Form = new PDF417RequestForm(movePosition.PDF417Codes, string.Format("Отсканируйте штрих-код PDF-417 (сканирование №{0} из {1})", i + 1, detailForm.Quantity));

                            if (pdf417Form.ShowDialog(this) == DialogResult.OK)
                            {
                                movePosition.PDF417Codes.Add(pdf417Form.PDF417);
                            }
                            else
                            {
                                Position_gridView.RefreshData();
