...
{
    public partial class TransferToShopEditorForm : XtraForm
    {
        ...
        /// Только чтение.
        /// </summary>
        private readonly TransferToShop act;
        /// <summary>
        ...
        /// </summary>
        public TransferToShopEditorForm()
        {
            try
            {
        ...
        public TransferToShopEditorForm(Documents documents, TransferToShop act) : this()
        {
            try
            {
        ...
        private void TransferToShopEditorForm_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void TransferToShopEditorForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (dataChanged)
        ...
        private void TransferToShopEditorForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
        ...
        private void SelectAll_simpleButton_Click(object sender, EventArgs e)
        {
            try
            {
                Source_gridView.SelectAll();
            }
            catch (Exception exception)
            {
                Program.Logger.Error("Во время выполнения операции произошла ошибка.", exception);
        ...
        private void setControlsEnable()
        {
            ...
            else
            {
                ...
                Text = "Просмотр акта передачи товара в торговый зал организации";
            }
        }
        /// <summary>
        /// Добавить позицию.
        ...
        private void addPosition()
        {
            try
            {
                #region Разрешение "multiselect"-а...
                // -----------------------------------------------------------------------------------------
                // Обработка для случая, когда в гриде остатков разрешён "multiselect".
                bool already = false;

                foreach (int inx in Source_gridView.GetSelectedRows())
                {
                    RestsPosition selected = (RestsPosition)Source_gridView.GetRow(inx);
                    ...
                    positions.Add(new MovePosition(MovementType.TransferToShop, selected, selected.Quantity));
                }

                if (already)
                {
                    XtraMessageBox.Show("Во время добавления были пропущены некоторые выбранные товары, " +
                                        "так как в позициях документа уже присутствует такая продукция.\r\n" +
                                        "В данном случае вам необходимо изменить количество уже выбранного товара.",
                                        "Операция выполнена частично", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                // -----------------------------------------------------------------------------------------
                #endregion Разрешение "multiselect"-а...

                #region Запрет "multiselect"-а...
                /* -----------------------------------------------------------------------------------------
                 * Обработка для случая, когда в гриде остатков запрещён "multiselect".
                 * ----------------------------------------------------------------------------------------- 
                ...
                positions.Add(new MovePosition(MovementType.TransferToShop, selected, selected.Quantity));
                ----------------------------------------------------------------------------------------- */
                #endregion Запрет "multiselect"-а...
