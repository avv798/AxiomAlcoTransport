...
{
	public partial class WaybillActForm : XtraForm
	{
        ...
        public WaybillActForm(Documents documents): this()
        {
            try
            {
        ...
        private void WaybillActForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Document_bindingSource.DataSource = documents.WaybillActs;
        ...
        private void WaybillActForm_Activated(object sender, EventArgs e)
        {
            try
            {
                ...
                Cursor = MdiParent.Cursor = Cursors.WaitCursor;
                MainRibbonControl.SelectedPage = WaybillAct_ribbonPage;
        ...
        private void WaybillActForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
        ...
        private void Document_gridView_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            try
            {
                ...
                WaybillAct current = (WaybillAct)Document_gridView.GetRow(e.RowHandle);
        ...
        private void Document_gridView_SelectionChanged(object sender, DevExpress.Data.SelectionChangedEventArgs e)
        {
            try
            {
                Cursor = MdiParent.Cursor = Cursors.WaitCursor;
                ...
                WaybillAct selected = (WaybillAct)Document_bindingSource.Current;
        ...
        private void Accept_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            actHandler(true);
        }
        private void Reject_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            actHandler(false);
        }
        private void Resend_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if ((selected.StatusEnum == WaybillActStatus.New) || (selected.StatusEnum == WaybillActStatus.Information))
                {
                    XtraMessageBox.Show("Переотправить подтверждение (или отказ) можно только по тем актам, " +
                ...
                if (DialogResult.Yes == XtraMessageBox.Show("Переотправить подтверждение (или отказ) для выбранного акта?",
                                                            "Подтверждение операции", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
                {
                    ...
                    documents.ResendConfirmAct(selected);
	    ...
	    private void changeDocumentReadingStatus(bool status)
	    {
            try
            {
                ...
                {
                    WaybillAct p = (WaybillAct)Document_gridView.GetRow(i);
        ...
        /// </summary>
        /// <param name="isAccepted">Тип акта.</param>
	    private void actHandler(bool isAccepted)
	    {
            try
            {
                if (Document_bindingSource.Current == null) return;
                ...
                if ((selected.StatusEnum == WaybillActStatus.New) && (selected.IsContentExist))
                {
                    (new WaybillActSingleForm(documents, selected, isAccepted)).ShowDialog();
                ...
                {
                    XtraMessageBox.Show("Подтвердить или отказать можно только акт расхождения, находящийся в статусе \"Новый\".\r\n" +
                                        "Для простых актов подтверждения или отказа (не для актов расхождения) дополнительных операций не требуется.",
                                        "Выполнение операции невозможно", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception exception)
