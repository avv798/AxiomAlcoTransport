...
{
	public partial class ProductionForm : XtraForm
	{
        ...
        public ProductionForm(Documents documents): this()
        {
            try
            {
        ...
        private void ProductionForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Document_bindingSource.DataSource = documents.Production;
        ...
        private void ProductionForm_Activated(object sender, EventArgs e)
        {
            try
            {
                ...
                Cursor = MdiParent.Cursor = Cursors.WaitCursor;
                MainRibbonControl.SelectedPage = Production_ribbonPage;
        ...
        private void RequestProductions_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                using (InnRequestForm form = new InnRequestForm("Укажите ИНН организации-производителя, " +
                                                                "перечень алкогольной продукции которой вы хотите получить."))
                {
                    ...
                    documents.RequestAP(form.Inn);

                    XtraMessageBox.Show("Запрос на получение перечня алкогольной продукции организации-производителя успешно отправлен.",
        ...
        private void Document_gridView_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            try
            {
                ...
                Production current = (Production)Document_gridView.GetRow(e.RowHandle);
        ...
        private void Document_gridView_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                Production selected = (Production)Document_bindingSource.Current;
        ...
        private void Garbage_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                List<Production> selected = Document_gridView.GetSelectedRows().Select(i => (Production)Document_gridView.GetRow(i)).ToList();
                                                                          ...
                                                                          , MessageBoxIcon.Question))
                {
                    ...
                    foreach (Production document in selected)
                    {
        ...
        private void Delete_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (DialogResult.Yes == XtraMessageBox.Show(string.Format("Удалить выбранные документы (в количестве {0} шт.) " +
                                                                          "без возможности дальнейшего восстановления?", selected.Count)
                                                                          , "Подтверждение операции"
                                                                          , MessageBoxButtons.YesNo
                                                                          , MessageBoxIcon.Question))
                {
                    ...
                    Program.Logger.Info(this, string.Format("Производится попытка безвозвратного удаления документов (в количестве {0} шт.)...", selected.Count));

                    foreach (Production document in selected)
                    {
                        documents.Delete(document, false);
                    }

                    Program.Logger.Info(this, string.Format("... попытка безвозвратного удаления документов (в количестве {0} шт.) успешно завершена.", selected.Count));
            ...
            {
                const string msg = "Во время безвозвратного удаления документов произошла ошибка.";
                Program.Logger.Error(msg, exception);
	    ...
	    private void changeDocumentReadingStatus(bool status)
	    {
            try
            {
                ...
                {
                    Production p = (Production)Document_gridView.GetRow(i);
