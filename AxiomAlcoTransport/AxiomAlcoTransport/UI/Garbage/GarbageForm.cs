...
{
	public partial class GarbageForm : XtraForm
	{
        ...
        public GarbageForm(Documents documents): this()
        {
            try
            {
        ...
        private void GarbageForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Document_bindingSource.DataSource = documents.Garbage;
        ...
        private void GarbageForm_Activated(object sender, EventArgs e)
        {
            try
            {
                ...
                Cursor = MdiParent.Cursor = Cursors.WaitCursor;
                MainRibbonControl.SelectedPage = Garbage_ribbonPage;
        ...
        private void GarbageForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
        ...
        private void Document_gridView_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                ADocument selected = (ADocument)Document_bindingSource.Current;
        ...
        private void Restore_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                List<ADocument> selected = Document_gridView.GetSelectedRows()
                                                            .Select(i => (ADocument)Document_gridView.GetRow(i))
                                                            .Distinct()
                                                            .ToList();

                if (selected.Count == 0) return;

                // ADocument.RemovePairFromList(selected);

                if (DialogResult.Yes == XtraMessageBox.Show(string.Format("Восстановить выбранные документы (в количестве {0} шт.)?", selected.Count)
                                                                          ...
                                                                          , MessageBoxIcon.Question))
                {
                    ...
                    Program.Logger.Info(this, string.Format("Производится попытка восстановления из корзины документов (в количестве {0} шт.)...", selected.Count));

                    foreach (ADocument document in selected)
                    {
                        documents.RestoreFromGarbage(document);
                    }

                    Program.Logger.Info(this, string.Format("... попытка восстановления из корзины документов (в количестве {0} шт.) успешно завершена.", selected.Count));
            ...
            {
                const string msg = "Во время восстановления документов из корзины произошла ошибка.";
