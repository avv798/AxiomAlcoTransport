...
{
	public partial class FormAForm : XtraForm
	{
        ...
        public FormAForm(Documents documents): this()
        {
            try
            {
        ...
        private void FormAForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Document_bindingSource.DataSource = documents.FormAList;
        ...
        private void FormAForm_Activated(object sender, EventArgs e)
        {
            try
            {
                ...
                Cursor = MdiParent.Cursor = Cursors.WaitCursor;
                MainRibbonControl.SelectedPage = FormA_ribbonPage;
        ...
        private void FormAForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
        ...
        private void RequestFormAs_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                using (PositionRequestForm form = new PositionRequestForm(documents, "Сначала выберите нужную товарно-транспортную накладную, " +
                                                                                     "затем выберите нужную позицию, по которой необходимо " +
                                                                                     "получить справку формы 'А'..."))
                {
                    ...
                    documents.RequestForm(FormsFormat.A, form.Position.FormARegId);

                    XtraMessageBox.Show("Запрос на получение справки формы 'А' успешно отправлен.",
                                        "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception exception)
            {
                const string msg = "Во время отправки запроса на получение справки формы 'А' произошла ошибка.";
        ...
        private void Document_gridView_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            try
            {
                ...
                FormA current = (FormA)Document_gridView.GetRow(e.RowHandle);
        ...
        private void Document_gridView_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                FormA selected = (FormA)Document_bindingSource.Current;
        ...
        private void Garbage_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                List<FormA> selected = Document_gridView.GetSelectedRows().Select(i => (FormA)Document_gridView.GetRow(i)).ToList();
                                                                          ...
                                                                          , MessageBoxIcon.Question))
                {
                    ...
                    foreach (FormA document in selected)
                    {
	    ...
	    private void changeDocumentReadingStatus(bool status)
	    {
            try
            {
                ...
                {
                    FormA p = (FormA)Document_gridView.GetRow(i);
