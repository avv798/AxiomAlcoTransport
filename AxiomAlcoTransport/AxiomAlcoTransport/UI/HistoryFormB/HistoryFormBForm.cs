...
{
	public partial class HistoryFormBForm : XtraForm
	{
        ...
        public HistoryFormBForm(Documents documents): this()
        {
            try
            {
        ...
        private void HistoryFormBForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Document_bindingSource.DataSource = documents.HistoryFormBList;
        ...
        private void HistoryFormBForm_Activated(object sender, EventArgs e)
        {
            try
            {
                ...
                Cursor = MdiParent.Cursor = Cursors.WaitCursor;
                MainRibbonControl.SelectedPage = HistoryFormB_ribbonPage;
        ...
        private void HistoryFormBForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
        ...
        private void RequestHistoryFormBs_barButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                using (RegIdRequestForm form = new RegIdRequestForm())
                {
                    ...
                    documents.RequestHistoryFormB(form.RegId);

                    SplashAccessor.Close();
                    Cursor = Cursors.Default;

                    XtraMessageBox.Show("Запрос на получение истории движения товара по форме 'Б' успешно отправлен.",
                                        "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception exception)
            {
                const string msg = "Во время отправки запроса на получение истории движения товара по форме 'Б' произошла ошибка.";
        ...
        private void Document_gridView_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            try
            {
                ...
                HistoryFormB current = (HistoryFormB)Document_gridView.GetRow(e.RowHandle);
        ...
        private void Document_gridView_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                HistoryFormB selected = (HistoryFormB)Document_bindingSource.Current;
	    ...
	    private void changeDocumentReadingStatus(bool status)
	    {
            try
            {
                ...
                {
                    HistoryFormB p = (HistoryFormB)Document_gridView.GetRow(i);
