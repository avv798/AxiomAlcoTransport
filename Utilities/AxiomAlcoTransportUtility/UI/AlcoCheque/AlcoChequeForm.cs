...
{
    public partial class AlcoChequeForm : XtraForm
    {
        ...
        /// Только чтение.
        /// </summary>
        private readonly BindingList<AlcoPosition> positions;
        /// <summary>
        /// Адрес УТМ.
        /// Только чтение.
        /// </summary>
        ...
        /// Только чтение.
        /// </summary>
        public BindingList<AlcoPosition> Positions
        {
        ...
        public AlcoChequeForm()
        {
            try
            {
                ...
                positions = new BindingList<AlcoPosition>();
        ...
        /// </summary>
        public AlcoChequeForm(string utmAddress, int timeout) : this()
        {
            try
            {
        ...
        private void AlcoChequeForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            Program.Logger.Info(this, "Форма закрывается.");
        }
        private void AlcoChequeForm_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Send_simpleButton_Click(object sender, EventArgs e)
        {
            try
            {
                Program.Logger.Info(this, "Попытка формирования и отправки алкогольного чека... ");
                ...
                XtraMessageBox.Show(string.Format("Алкогольный чек (направление: '{0}'; количество позиций: {1}; общая сумма: {2} руб.) успешно отправлен на сервер УТМ.",
                ...
                Program.Logger.Info(this, "... формирование и отправка алкогольного чека завершена.");
            }
            catch (Exception exception)
            {
        ...
        private void Add_simpleButton_Click(object sender, EventArgs e)
        {
            try
            {
                AlcoPosition position;

                using (AlcoPositionForm form = new AlcoPositionForm())
                {
                ...
                if (positions.Any(x => (x.Barcode.ToLower() == position.Barcode.ToLower())))
                {
                    XtraMessageBox.Show("Позиция с указанным штрих-кодом PDF-417 уже присутствует в списке.", "Повтор позиции", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void Remove_simpleButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AlcoPosition selected = (AlcoPosition)Document_bindingSource.Current;
        ...
        private AlcoCheque sendCheque()
        {
            ...
            #region Creating...
            {
                ...
                cheque = new AlcoCheque
                                {
                                    ...
                                    ProcessDateTime =
                                        new DateTime(date.Year, date.Month, date.Day, time.Hour, time.Minute, time.Second, 0),
                ...
                Program.Logger.Info(this, "\t... создание алкогольного чека завершено... ");
            }
            #endregion Creating...

            #region Xml...
            {
                Program.Logger.Info(this, "\t... начато формирование xml-документа алкогольного чека... ");

                xmlDocument = cheque.BuildXmlDocument();

                Program.Logger.Info(this, string.Format("\t... формирование xml-документа алкогольного чека завершено; текст xml-документа: '{0}'... ", xmlDocument.OuterXml));
            }
            #endregion Xml...
            ...
            { 
                ...
                string filename = string.Format("{0}\\informa.util.egais.alco.{1}.xml", ACheque.PathOutRequest, DateTime.Now.ToString("yyyyMMddHHmmssfff"));

                Program.Logger.Info(this, string.Format("\t... попытка сохранения xml-документа алкогольного чека в файле '{0}'... ", filename));
                xmlDocument.Save(filename);
                Program.Logger.Info(this, string.Format("\t... сохранения xml-документа алкогольного чека в файле '{0}'... ", filename));
            }
            #endregion File...
            ...
            #region Sending...
            {
                ...
                Program.Logger.Info(this, string.Format("\t... отправка xml-документа алкогольного чека по адресу '{0}' завершена... ", address));
